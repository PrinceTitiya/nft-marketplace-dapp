...
export default function SellNft() {
  ...
  const fetchProceeds = async () => {
    ...
    try {
      ...
      const userProceeds = await contract.getProceeds(address);
  ...
  const handleListItem = async () => {
    ...
    try {
      ...
      const existingListing = await marketplaceContract.getListing(nftAddress, tokenId);
      if (existingListing.price > 0) { setStatus("⚠️ This NFT is already listed!"); return; }
      ...
      const tx = await marketplaceContract.listItem(nftAddress, tokenId, ethers.parseEther(price));
      await tx.wait();
  ...
  const handleCancelListing = async () => {
    ...
    try {
      ...
      const signer = await provider.getSigner();
      const marketplaceContract = new ethers.Contract(marketplaceAddress, marketplaceAbi, signer);
      const existingListing = await marketplaceContract.getListing(cancelNftAddress, cancelTokenId);
      if (existingListing.price.toString() === "0") {
        setStatus("⚠️ This NFT is not listed"); setShowConfirmCancel(false); return;
      }
      const tx = await marketplaceContract.cancelListing(cancelNftAddress, cancelTokenId);
      await tx.wait();
      setStatus("✅ NFT successfully canceled!");
      setCancelNftAddress(""); setCancelTokenId(""); setShowConfirmCancel(false);
    } catch (err) {
      console.error(err);
      setStatus("❌ Error: " + err.message);
  ...
  async function checkListing() {
    ...
    try {
      ...
      const listingData = await contract.getListing(updateNftAddress, updateTokenId);
      if (listingData.price > 0) {
        setListing({ price: ethers.formatEther(listingData.price), seller: listingData.seller });
        setStatus("");
      } else {
        setListing(null); setStatus("❌ This NFT is not listed.");
      }
    } catch (err) {
      console.error(err); setStatus("⚠️ Error checking listing.");
    }
  }
  ...
  async function updateListing() {
    ...
    try {
      ...
      const signer = await provider.getSigner();
      const contract = new ethers.Contract(marketplaceAddress, marketplaceAbi, signer);
      const tx = await contract.updateListing(updateNftAddress, updateTokenId, ethers.parseEther(newPrice));
      await tx.wait();
      setStatus("✅ Listing updated successfully!");
      setListing({ ...listing, price: newPrice });
    } catch (err) {
      console.error(err); setStatus(" Failed to update listing.");
    }
  }

  useEffect(() => setMounted(true), []);
  if (!mounted) return (
    <div style={{ minHeight: "100vh", background: "#0f0c29", display: "flex", alignItems: "center", justifyContent: "center", color: "#6366f1", fontSize: "1.1rem" }}>
      Loading...
    </div>
  );

  const toastStyle = toast.type === "success" ? styles.toastSuccess : toast.type === "error" ? styles.toastError : styles.toastInfo;
    ...
    <>
      ...
      <div style={styles.pageWrapper}>
        ...
        <div style={styles.content}>
          ...
          <p style={styles.heroSub}>List, cancel, update, or withdraw proceeds from your NFTs in one place.</p>

          {/* ── TOP GRID: List + Cancel ── */}
          <div style={{ ...styles.grid2, ...(typeof window !== "undefined" && window.innerWidth < 640 ? { gridTemplateColumns: "1fr" } : {}) }}>
            ...
            <div style={styles.card}>
              ...
              <button
                ...
                style={{ ...styles.btnIndigo, ...(!isConnected ? styles.btnDisabled : {}) }}
              >
                {isConnected ? " List NFT" : "Connect Wallet to List"}
              </button>
            </div>
            ...
            <div style={styles.card}>
              ...
              <button
                ...
                disabled={!isConnected}
                style={{ ...styles.btnRed, ...(!isConnected ? styles.btnDisabled : {}) }}
              >
          ...
          <div style={{ ...styles.card, marginBottom: "24px" }}>
            ...
            <button
              ...
              style={{ ...styles.btnIndigo, marginBottom: "16px", background: "linear-gradient(135deg, #f59e0b, #d97706)", boxShadow: "0 4px 16px rgba(245,158,11,0.35)" }}
            >
              Check Listing
            </button>
            ...
            {listing && (
              <>
                <div style={styles.infoBox}>
                  <div><strong>Listed by:</strong> <span style={{ fontFamily: "monospace", fontSize: "0.82rem" }}>{listing.seller}</span></div>
                  <div> <strong>Current Price:</strong> {listing.price} ETH</div>
                </div>
                <input
                  type="text"
            ...
            <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
              ...
              <button
                ...
                style={{
                  ...
                  width: "auto",
                  padding: "10px 22px",
                  ...(!proceeds || parseFloat(proceeds) === 0 ? styles.btnDisabled : {}),
                }}
              >
                Withdraw
              </button>
      ...
      {showConfirmCancel && (
        <div style={styles.modalOverlay}>
          <div style={styles.modalBox}>
            ...
            <p style={{ color: "#e2e8f0", fontSize: "1.05rem", fontWeight: "600", marginBottom: "8px" }}>
              Cancel this listing?
            </p>
            <p style={{ color: "#94a3b8", fontSize: "0.85rem", marginBottom: "24px" }}>
              This will remove your NFT from the marketplace. You can re-list it anytime.
            </p>
